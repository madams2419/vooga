...
public class HexTile {
	...
	public HexTile(double x, double y, double side) {
		...
		for (int i = 0; i < POINTS; i += 2) {
			...
			points[i] = newX;
			points[i + 1] = newY;
			connectionPoints[i] = newX + side * Math.cos(Math.toRadians(startAngle + HEX));
			connectionPoints[i + 1] = newY- side * Math.sin(Math.toRadians(startAngle + HEX));;
			startAngle += HEX;
		}
...
//		}
	...
	public void setHandler(EventHandler<MouseEvent> e){
